...
async def test_get_devices(test_client: AsyncTestClient[Litestar]) -> None:
    ...
    encrypted_data = encryption_helper.encrypt_msg(data, TEST_CLIENT_ID_1)
    encrypted_data |= {'client_id': TEST_CLIENT_ID_1}
    ...
    encrypted_data = encryption_helper.encrypt_msg(data, TEST_CLIENT_ID_2)
    encrypted_data |= {'client_id': TEST_CLIENT_ID_2}
    response = await test_client.post('/register', json=encrypted_data)
